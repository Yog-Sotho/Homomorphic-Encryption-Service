...
pub async fn get_me(
    ...
    let row: Option<(String, String, bool, String, chrono::NaiveDateTime, Option<String>, Option<i64>)> = sqlx::query_as(
        "SELECT u.id, u.email, u.email_verified, u.password_hash, u.created_at, \
         GROUP_CONCAT(o.provider) AS oauth_providers, d.count AS daily_usage_count \
         FROM users u \
    ...
    let (id, email, email_verified, password_hash, created_at, oauth_providers_concat, daily_usage_count) = match row {
        None => return Err(AppError::not_found("User not found")),
        Some(r) => r,
    };

    let oauth_providers: Vec<String> = oauth_providers_concat
        .map(|s| s.split(',').filter(|item| !item.is_empty()).map(|item| item.to_string()).collect())
        .unwrap_or_default();

    let count = daily_usage_count.unwrap_or(0);
