...
mod tests {
    ...
    async fn test_token_length_validation() {
        ...
        // 1. Reset password with overly long token
        let req = web::Json(ResetPasswordRequest {
            token: "a".repeat(257),
            new_password: "Password123!".to_string(),
        });
        let res = reset_password(pool_data.clone(), req).await;
        assert!(res.is_err());
        match res.err().unwrap() {
        ...
        // 2. Refresh token with overly long token
        let req = web::Json(RefreshRequest {
            refresh_token: "a".repeat(257),
        });
        let res = refresh_token_endpoint(pool_data.clone(), config_data, req).await;
        assert!(res.is_err());
        match res.err().unwrap() {
        ...
        // 3. Logout with overly long token
        let req = web::Json(LogoutRequest {
            refresh_token: "a".repeat(257),
        });
        let res = logout(pool_data, req).await;
        assert!(res.is_err());
        match res.err().unwrap() {
